 ...
 * View all datasets.
 */
app.get('/admin/dataset', [checkAccess, loadDataSet], function(req, res, next) {
 ...
 */
app.get('/admin/dataset/edit/:set', [checkAccess, loadDataSet], function(req, res, next) {
    res.render(__dirname + '/templates/dataset_edit.hbs', {locals: {
        pageTitle: 'Manage data sets',
        title: 'Manage data sets',
        content: 'TODO: list data sets',
        dataSetName: req.params.set
    }});
});
 ...
 */
app.get('/admin/dataset/edit.js', function(req, res, next) {
    var script = '';
    if (!script.length) {
